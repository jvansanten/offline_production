 ...
 *
 *
 * $Id: I3CLSimLightSourceToStepConverter.cxx 180094 2020-05-13 14:46:40Z jvansanten $
 *
 * @file I3CLSimLightSourceToStepConverter.cxx
 * @version $Revision: 180094 $
 * @date $Date: 2020-05-13 08:46:40 -0600 (Wed, 13 May 2020) $
...
bp::tuple GetConversionResultWithBarrierInfoAndMarkers(I3CLSimLightSourceToStepConverterAsync &obj, double timeout)
{
    ...
    bp::list markers(std::get<1>(ret));
    bp::dict particleHistories;
    for (const auto &item : std::get<2>(ret)) {
        particleHistories[item.first] = item.second;
    }

    return bp::make_tuple(std::get<0>(ret), markers, particleHistories, barrierWasReset);
}
